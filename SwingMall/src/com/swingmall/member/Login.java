...
public class Login extends Page{
	...
	// 유효성 체크 (입력하지 않은 필드가 있는지여부에 따른 피드백*)
	public void validCheck(ShopMember shopMember) {
		if(shopMember.getMid().length()<1) {// 문자열의 길이가 0이라면..
			...
			if(login(shopMember)==null) {
				...
				getShopMain().setHasSession(true);
			}
		}		
	}
	
	// 회원 로그인 처리메서드 정의 : 로그인* 성공 후 Home 을 보여줄 예정임
	// 아래의 메서드가 ShopMember 를 반환하므로, 만일 로그인 실패한 경우에는
	//null을 반환받아 간다!! 
	public ShopMember login(ShopMember shopMember) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ShopMember vo = null;		// 로그인 성공 후 회원의 모든 정보를 담게 될 VO
	...
	// 로그아웃 처리 
	// 1. hasSession의 값을 false 2. 버튼의 배경색 빼기 3. 버튼의 텍스트 login으로 바꾸기
	public void logout() {
